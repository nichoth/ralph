...
1. READ: At the start of every session, read `specs/prd.json`, `progress.log`
   (especially the `## Codebase Patterns` section), `NOTES/architecture.md`,
   and `NOTES/product.md`.
...
9. COMMIT: Once the full suite and lint pass, commit with a descriptive message
   like `FEATURE: [TaskID] - [Description]` (or `FIX:`, `TEST:`, `CHORE:` as
   appropriate).
